...
function CreateListing() {
  ...
  const onSubmit = async (e) => {
    ...
    // location && (formDataCopy.location = location); as in some cases it might be misleading as geoloaction sometimes removes the exact adderess
  ...
  const onMutate = (e) => {
    ...
    if (!e.target.files) {
      setFormData((prevState) => ({
        ...prevState,
        [e.target.id]: boolean ?? e.target.value, //nullish operator which means that if value on left side is null then right side value will be used instead
      }));
    }
  };
  if (loading) {
